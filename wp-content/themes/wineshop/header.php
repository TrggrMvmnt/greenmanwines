...
<head>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1<?php echo (themerex_get_theme_option('responsive_layouts') == 'yes' ? ', maximum-scale=1' : ''); ?>">
...
>
	...
	if (themerex_get_custom_option('menu_toc_home')=='yes')
		echo trim(themerex_sc_anchor(array(
			'id' => "toc_home",
			'title' => esc_html__('Home', 'themerex'),
			'description' => esc_html__('{{Return to Home}} - ||navigate to home page of the site', 'themerex'),
			'icon' => "icon-home",
			'separator' => "yes",
			'url' => home_url('/'))
			)); 
	if (themerex_get_custom_option('menu_toc_top')=='yes')
		echo trim(themerex_sc_anchor(array(
			'id' => "toc_top",
			'title' => esc_html__('To Top', 'themerex'),
			'description' => esc_html__('{{Back to top}} - ||scroll to top of the page', 'themerex'),
			'icon' => "icon-double-up",
			'separator' => "yes")
			)); 
	?>
	...
	<?php
		$class = $style = '';
	//	if ($body_style=='boxed' || $body_style=='wide' ||  themerex_get_custom_option('bg_image_load')=='always') {
	...
	<div class="body_wrap<?php echo balanceTags($class ? ' '.esc_attr($class) : ''); ?>"<?php echo balanceTags($style ? ' style="'.esc_attr($style).'"' : ''); ?>>
		...
		if ($video_bg_show) {
			...
			if (!empty($youtube)) {
				?>
				<div class="video_bg<?php echo balanceTags($overlay ? ' video_bg_overlay' : ''); ?>" data-youtube-code="<?php echo esc_attr($youtube); ?>"></div>
				<?php
			} else if (!empty($video)) {
				$info = pathinfo($video);
