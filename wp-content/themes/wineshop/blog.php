...
echo balanceTags($container_start) . ($container2_start);
...
echo balanceTags($container2_end) . ($container_end);
...
if (!$post_number) { 
	...
	if ($show_filters == 'categories') {			// Use categories as filter items
		...
		if (is_array($portfolio_list) && count($portfolio_list) > 0) {
			$filters .= '<a href="#" data-filter="*" class="'.esc_attr($filter_button_classes . ($portfolio_parent==$cur_term_id ? ' active' : '')) . '">' . esc_html__('All', 'themerex').'</a>';
		...
		if (is_array($flt_ids) && count($flt_ids) > 0) {
			$filters .= '<a href="#" data-filter="*" class="'.esc_attr($filter_button_classes).' active">'.esc_html__('All', 'themerex').'</a>';
	...
	if ($filters) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function () {
				...
				jQuery(".isotope_filters").append('<?php echo balanceTags($filters); ?>');
			});
		</script>
		<?php
	}
	...
	if (in_array($pagination, array('viewmore', 'infinite'))) {
		if ($page_number < $wp_query->max_num_pages) {
			...
			<div id="viewmore" class="pagination_wrap pagination_<?php echo esc_attr($pagination); ?>">
				<a href="#" id="viewmore_link" class="theme_button viewmore_button"><span class="icon-spin3 animate-spin viewmore_loading"></span><span class="viewmore_text_1"><?php esc_html_e('LOAD MORE', 'themerex'); ?></span><span class="viewmore_text_2"><?php esc_html_e('Loading ...', 'themerex'); ?></span></a>
