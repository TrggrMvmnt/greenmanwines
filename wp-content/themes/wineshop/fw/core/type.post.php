		...
		if (isset($THEMEREX_GLOBALS['post_meta_box']) && $THEMEREX_GLOBALS['post_meta_box']['page']=='post') {
			$THEMEREX_GLOBALS['post_meta_box']['fields'] = array(
					...
					"info_reviews_1" => array(
						"title" => esc_html__('Reviews criterias for this post', 'themerex'),
						"override" => "post",
						"divider" => false,
						"desc" => wp_kses( __('In this section you can put your reviews marks', 'themerex'), $THEMEREX_GLOBALS['allowed_tags'] ),
						"class" => "reviews_meta",
						"type" => "info"),
					"show_reviews" => array(
					...
					"reviews_marks" => array(
						"title" => esc_html__('Reviews marks',  'themerex'),
						"override" => "post",
						"desc" => wp_kses( __("Marks for this review", 'themerex'), $THEMEREX_GLOBALS['allowed_tags'] ),
...
if (!function_exists('themerex_post_add_options_column')) {
	...
	function themerex_post_add_options_column( $columns ){
		$columns['theme_options'] = esc_html__('Theme Options', 'themerex');
		return $columns;
	}
}
			...
			if (is_array($props) && count($props) > 0) {
				foreach ($props as $prop_name=>$prop_value) {
					if (!themerex_is_inherit_option($prop_value) && (!isset($THEMEREX_GLOBALS['options'][$prop_name]['type']) || $THEMEREX_GLOBALS['options'][$prop_name]['type']!='hidden')) {
						...
						$options .= '<div class="themerex_options_prop_row"><span class="themerex_options_prop_name">' . esc_html($prop_title) . '</span>&nbsp;=&nbsp;<span class="themerex_options_prop_value">' . (is_array($prop_value) ? esc_html__('[Complex Data]', 'themerex') : '"' . esc_html(themerex_strshort($prop_value, 80)) . '"') . '</span></div>';
					}
				}
			}
		}
...
if ( !function_exists( 'themerex_post_list_post_types' ) ) {
	...
	function themerex_post_list_post_types($list) {
		$list['post'] = esc_html__('Post', 'themerex');
		$list['page'] = esc_html__('Page', 'themerex');
		return $list;
	}
}
...
if ( !function_exists( 'themerex_post_get_blog_title' ) ) {
	...
	function themerex_post_get_blog_title($title, $page) {
		...
		if ( $page == 'blog' )
			$title = esc_html__( 'All Posts', 'themerex' );
		else if ( $page == 'author' ) {
			$curauth = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
			$title = sprintf(esc_html__('Author page: %s', 'themerex'), $curauth->display_name);
		} else if ( $page == 'error404' )
			$title = esc_html__('URL not found', 'themerex');
		else if ( $page == 'search' )
			$title = sprintf( esc_html__( 'Search: %s', 'themerex' ), get_search_query() );
		else if ( $page == 'archives_day' )
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'themerex' ), themerex_get_date_translations(get_the_date()) );
		else if ( $page == 'archives_month' )
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'themerex' ), themerex_get_date_translations(get_the_date( 'F Y' )) );
		else if ( $page == 'archives_year' )
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'themerex' ), get_the_date( 'Y' ) );
		 else if ( $page == 'category' )
			$title = sprintf( esc_html__( '%s', 'themerex' ), single_cat_title( '', false ) );
		else if ( $page == 'tag' )
			$title = sprintf( esc_html__( 'Tag: %s', 'themerex' ), single_tag_title( '', false ) );
		else if ( $page == 'attachment' )
			$title = sprintf( esc_html__( 'Attachment: %s', 'themerex' ), themerex_get_post_title());
		else if ( $page == 'single' )
