...
if (!function_exists('themerex_show_breadcrumbs')) {
	function themerex_show_breadcrumbs($args=array()) {
		...
		$args = array_merge( array(
			'home' => esc_html__('Home', 'themerex'),		// Home page title (if empty - not showed)
							...
							if ($level > $args['max_levels'])
								...
								$rez_all .= (!empty($rez_all) ? $args['delimiter'] : '') 
										. '<a class="breadcrumbs_item all" href="' . get_permalink($tpl_id) . '">' 
											. sprintf(esc_html__('All %s', 'themerex'), themerex_get_post_format_name($post_format, false)) 
										. '</a>';
						}
					}
...
if (!function_exists('themerex_show_pagination')) {
	function themerex_show_pagination($args=array()) {
		$args = array_merge(array(
			...
			'show_pages' => 5,
			'pages_in_group' => 10,
			'pages_text' => '', 		//esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'themerex'),
			'current_text' => "%PAGE_NUMBER%",
			'page_text' => "%PAGE_NUMBER%",
			'first_text'=> esc_html__('&laquo; First', 'themerex'),
			'last_text' => esc_html__("Last &raquo;", 'themerex'),
			'prev_text' => esc_html__("&laquo; Prev", 'themerex'),
			'next_text' => esc_html__("Next &raquo;", 'themerex'),
			'dot_text' => "&hellip;",
			'before' => '',
			'after' => '',
			),  is_array($args) ? $args 
...
if (!function_exists('themerex_show_pagination_single')) {
	function themerex_show_pagination_single( $args ) {
		...
		if ( $wp_query->max_num_pages < 2 && ( is_home() || is_archive() || is_search() ) )
			return;
		echo balanceTags($args['before']);
		if ( is_single() ) {
			?>
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'themerex' ); ?></h3>
			<?php
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if (empty($opt['pages_text'])) 
						$opt['pages_text'] = esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'themerex');
...
if ( !function_exists( 'themerex_callback_view_more_posts' ) ) {
	function themerex_callback_view_more_posts() {
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			$response['error'] = esc_html__('Wrong query arguments', 'themerex');
		}
...
if (!function_exists('themerex_get_calendar')) {
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) )
				$calendar_output .= '<a class="day_wrap" href="' . esc_url(!empty($posts[$day]['link']) ? $posts[$day]['link'] : get_day_link($thisyear, $thismonth, $day)) . '" title="' . esc_attr( is_int($posts[$day]['titles']) ? $posts[$day]['titles'].' '.esc_html__('items', 'themerex') : $posts[$day]['titles'] ) . "\">$day</a>";
			else
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">' 
...
if (!function_exists('themerex_output_single_comment')) {
	function themerex_output_single_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php esc_html_e( 'Trackback:', 'themerex' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'themerex' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php esc_html_e( 'Pingback:', 'themerex' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'themerex' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							<span class="comment_author"><?php echo balanceTags($author_id ? '<a href="'.esc_url($author_link).'">' : '') . comment_author() . ($author_id ? '</a>' : ''); ?></span>
							<span class="comment_date"><span class="comment_date_label"><?php esc_html_e('Posted', 'themerex'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
							<span class="comment_time"><?php echo get_comment_date(get_option('time_format')); ?></span>
						</div>
						<div class="comment_text_wrap">
							<?php if ( $comment->comment_approved == 0 ) { ?>
							<div class="comment_not_approved"><?php esc_html_e( 'Your comment is awaiting moderation.', 'themerex' ); ?></div>
							<?php } ?>
	...
	function themerex_callback_registration_user() {
		...
		if ( is_wp_error($id) ) {
			...
			if (in_array($notify, array('both', 'admin', 'yes'))) {
				$subj = sprintf(esc_html__('Site %s - New user registration: %s', 'themerex'), esc_html(get_bloginfo('site_name')), esc_html($user_name));
				$msg = "\n".esc_html__('New registration:', 'themerex')
					."\n".esc_html__('Name:', 'themerex').' '.esc_html($user_name)
					."\n".esc_html__('E-mail:', 'themerex').' '.esc_html($user_email)
			...
			if (in_array($notify, array('both', 'user', 'yes'))) {
				$subj = sprintf(esc_html__('Welcome to "%s"', 'themerex'), get_bloginfo('site_name'));
				$msg = "\n".esc_html__('Your registration data:', 'themerex')
					."\n".esc_html__('Name:', 'themerex').' '.esc_html($user_name)
					."\n".esc_html__('E-mail:', 'themerex').' '.esc_html($user_email)
					."\n".esc_html__('Password:', 'themerex').' '.esc_html($user_pwd)
