	...
	function themerex_attachment_theme_setup() {
		...
		if (function_exists('themerex_utils_require_data')) {
			themerex_utils_require_data( 'taxonomy', 'media_folder', array(
				...
				'hierarchical' 		=> true,
				'labels' 			=> array(
					'name'              => esc_html__('Media Folders', 'themerex'),
					'singular_name'     => esc_html__('Media Folder', 'themerex'),
					'search_items'      => esc_html__('Search Media Folders', 'themerex'),
					'all_items'         => esc_html__('All Media Folders', 'themerex'),
					'parent_item'       => esc_html__('Parent Media Folder', 'themerex'),
					'parent_item_colon' => esc_html__('Parent Media Folder:', 'themerex'),
					'edit_item'         => esc_html__('Edit Media Folder', 'themerex'),
					'update_item'       => esc_html__('Update Media Folder', 'themerex'),
					'add_new_item'      => esc_html__('Add New Media Folder', 'themerex'),
					'new_item_name'     => esc_html__('New Media Folder Name', 'themerex'),
					'menu_name'         => esc_html__('Media Folders', 'themerex'),
				),
				'query_var'			=> true,
				'rewrite' 			=> true,
	...
	function themerex_attachment_view_filters($settings, $post=null) {
		...
		if (is_array($taxes) && count($taxes) > 0) {
			foreach ($taxes as $tax) {
				...
				if (count($terms) > 0) {
					$settings['mimeTypes'][$tax.'.'] = esc_html__('Media without folders', 'themerex');
...
if (!function_exists('themerex_attachment_view_compat')) {
	...
	function themerex_attachment_view_compat($form_fields, $post=null) {
		...
		if (isset($form_fields['media_folder'])) {
			...
			$html = '';
			$html .= '<input type="text" class="text" id="media_folder_'.esc_attr($id).'" name="media_folder___new" value="" placeholder="' . esc_attr__('Create new folder', 'themerex') . '"' . ($readonly) . ' ' . ($aria_required) . ' />';
			if (is_array($terms) && count($terms) > 0) {
