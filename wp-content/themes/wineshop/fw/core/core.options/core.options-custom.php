	...
	switch ($field['type']) {
		...
		case 'mediamanager':
			...
			$output .= '<a id="'.esc_attr($id).'" class="button mediamanager"
				...
				data-choose="'.esc_attr(isset($field['multiple']) && $field['multiple'] ? esc_html__( 'Choose Images', 'themerex') : esc_html__( 'Choose Image', 'themerex')).'"
				data-update="'.esc_attr(isset($field['multiple']) && $field['multiple'] ? esc_html__( 'Add to Gallery', 'themerex') : esc_html__( 'Choose Image', 'themerex')).'"
				data-multiple="'.esc_attr(isset($field['multiple']) && $field['multiple'] ? 'true' : 'false').'"
				data-linked-field="'.esc_attr($field['media_field_id']).'"
				onclick="themerex_show_media_manager(this); return false;"
				>' . (isset($field['multiple']) && $field['multiple'] ? esc_html__( 'Choose Images', 'themerex') : esc_html__( 'Choose Image', 'themerex')) . '</a>';
			break;
	}
