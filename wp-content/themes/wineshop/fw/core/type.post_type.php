	...
	function themerex_post_type_theme_setup() {
		...
		if ($override_key) {
			...
			$THEMEREX_GLOBALS['post_meta_box'] = array(
				'id' => 'post-meta-box',
				'title' => esc_html__('Post Options', 'themerex'),
				'page' => $post_type,
				'context' => 'normal',
				'priority' => 'high',
				'fields' => array()
...
if (!function_exists('themerex_post_type_show_meta_box')) {
	function themerex_post_type_show_meta_box() {
		...
		echo '<input type="hidden" name="meta_box_post_nonce" value="' .esc_attr($THEMEREX_GLOBALS['admin_nonce']).'" />';
...
if (!function_exists('themerex_post_type_save_options')) {
	...
	function themerex_post_type_save_options($post_id) {
		...
		// verify nonce
		if (!isset($_POST['meta_box_post_nonce']) || !wp_verify_nonce($_POST['meta_box_post_nonce'], $THEMEREX_GLOBALS['admin_url'])) {
			return $post_id;
		}
		...
		if ( !empty($post->ID) && $post_id==$post->ID ) {
			themerex_get_post_views($post_id);
			themerex_get_post_likes($post_id);
		}
	}
}
?>