	...
	function themerex_get_date_or_difference($dt1, $dt2=null, $max_days=-1) {
		...
		if (is_numeric($dt1n) && is_numeric($dt2n)) {
			...
			if (abs($days) < $max_days)
				return sprintf($days >= 0 ? esc_html__('%s ago', 'themerex') : esc_html__('in %s', 'themerex'), themerex_get_date_difference($days >= 0 ? $dt1 : $dt2, $days >= 0 ? $dt2 : $dt1));
			else
...
if (!function_exists('themerex_get_date_difference')) {
	function themerex_get_date_difference($dt1, $dt2=null, $short=1, $sec = false) {
		...
		if (is_numeric($dt1n) && is_numeric($dt2n)) {
			...
			$rez = '';
			if ($months > 0 && $short == 2)
				$rez .= ($rez!='' ? ' ' : '') . sprintf($months > 1 ? esc_html__('%s months', 'themerex') : esc_html__('%s month', 'themerex'), $months);
			if ($days > 0 && ($short < 2 || $rez==''))
				$rez .= ($rez!='' ? ' ' : '') . sprintf($days > 1 ? esc_html__('%s days', 'themerex') : esc_html__('%s day', 'themerex'), $days);
			if ((!$short || $rez=='') && $hours > 0)
				$rez .= ($rez!='' ? ' ' : '') . sprintf($hours > 1 ? esc_html__('%s hours', 'themerex') : esc_html__('%s hour', 'themerex'), $hours);
			if ((!$short || $rez=='') && $min > 0)
				$rez .= ($rez!='' ? ' ' : '') . sprintf($min > 1 ? esc_html__('%s minutes', 'themerex') : esc_html__('%s minute', 'themerex'), $min);
			if ($sec || $rez=='')
				$rez .=  $rez!='' || $sec ? (' ' . sprintf($diff > 1 ? esc_html__('%s seconds', 'themerex') : esc_html__('%s second', 'themerex'), $diff)) : esc_html__('less then minute', 'themerex');
			return $rez;
		} else
			return $dt1;
	}
...
if (!function_exists('themerex_get_date_translations')) {
	function themerex_get_date_translations($dt) {
		return str_replace(
			...
			array(
				esc_html__('January', 'themerex'),
				esc_html__('February', 'themerex'),
				esc_html__('March', 'themerex'),
				esc_html__('April', 'themerex'),
				esc_html__('May', 'themerex'),
				esc_html__('June', 'themerex'),
				esc_html__('July', 'themerex'),
				esc_html__('August', 'themerex'),
				esc_html__('September', 'themerex'),
				esc_html__('October', 'themerex'),
				esc_html__('November', 'themerex'),
				esc_html__('December', 'themerex'),
				esc_html__('Jan', 'themerex'),
				esc_html__('Feb', 'themerex'),
				esc_html__('Mar', 'themerex'),
				esc_html__('Apr', 'themerex'),
				esc_html__('May', 'themerex'),
				esc_html__('Jun', 'themerex'),
				esc_html__('Jul', 'themerex'),
				esc_html__('Aug', 'themerex'),
				esc_html__('Sep', 'themerex'),
				esc_html__('Oct', 'themerex'),
				esc_html__('Nov', 'themerex'),
				esc_html__('Dec', 'themerex'),
			),
			$dt);
	}
}
