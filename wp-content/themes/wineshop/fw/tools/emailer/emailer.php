	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			themerex_admin_add_menu_item('management', array(
				'page_title' => esc_html__('Emailer', 'themerex'),
				'menu_title' => esc_html__('Emailer', 'themerex'),
	...
	function build_page() {
		...
		if ( isset($_POST['emailer_subject']) ) {
			do {
				...
				if (empty($subject)) {
					$this->error = esc_html__('Subject can not be empty! Operation canceled!', 'themerex');
					break;
				}
				$message = themerex_get_value_gp('emailer_message');
				if (empty($message)) {
					$this->error = esc_html__('Message can not be empty! Operation canceled!', 'themerex');
					break;
				}
				...
				if (count($subscribers)==0) {
					$this->error = esc_html__('Subscribers lists are empty! Operation canceled!', 'themerex');
					break;
				}
				...
				if (empty($sender_email)) {
					$this->error = esc_html__('Sender email is empty! Operation canceled!', 'themerex');
					break;
				}
				...
				if ($subscribers_update && $group!='none') {
					...
					$add_msg = esc_html__(' The subscriber list is updated', 'themerex');
				} else if ($subscribers_delete && $group!='none') {
					unset($this->subscribers[$group]);
					update_option('themerex_emailer_subscribers', $this->subscribers);
					$add_msg = esc_html__(' The subscriber list is cleared', 'themerex');
				}
				$this->success = esc_html__('E-Mail was send successfull!', 'themerex') . $add_msg;
			} while (false);
		}

		?>
		<div class="trx_emailer">
			<h2 class="trx_emailer_title"><?php esc_html_e('ThemeREX Emailer', 'themerex'); ?></h2>
			...
			<form id="trx_emailer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						<legend> <?php esc_html_e('Letter data', 'themerex'); ?> </legend>
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_subject">
								<label for="emailer_subject"><?php esc_html_e('Subject:', 'themerex'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_attachment">
								<label for="emailer_attachment"><?php esc_html_e('Attachment:', 'themerex'); ?></label>
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						<legend> <?php esc_html_e('Subscribers', 'themerex'); ?> </legend>
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_group">
								<label for="emailer_group"><?php esc_html_e('Select group:', 'themerex'); ?></label>
								<select name="emailer_group" id="emailer_group">
									<option value="none"<?php echo balanceTags($group=='none' ? ' selected="selected"' : ''); ?>><?php esc_html_e('- Select group -', 'themerex'); ?></option>
									<?php
								...
								<input type="checkbox" name="emailer_subscribers_update" id="emailer_subscribers_update" value="1"<?php echo balanceTags($subscribers_update ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_update" class="inline" title="<?php esc_html_e('Update the subscribers list for selected group', 'themerex'); ?>"><?php esc_html_e('Update', 'themerex'); ?></label>
								<input type="checkbox" name="emailer_subscribers_clear" id="emailer_subscribers_clear" value="1"<?php echo balanceTags($subscribers_clear ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_clear" class="inline" title="<?php esc_html_e('Clear this group from not confirmed emails after send', 'themerex'); ?>"><?php esc_html_e('Clear', 'themerex'); ?></label>
								<input type="checkbox" name="emailer_subscribers_delete" id="emailer_subscribers_delete" value="1"<?php echo balanceTags($subscribers_delete ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_delete" class="inline" title="<?php esc_html_e('Delete this group after send', 'themerex'); ?>"><?php esc_html_e('Delete', 'themerex'); ?></label>
							</div>
							<div class="trx_emailer_field trx_emailer_subscribers2">
								<label for="emailer_subscribers" class="big"><?php esc_html_e('List of recipients:', 'themerex'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_sender_name">
								<label for="emailer_sender_name"><?php esc_html_e('Sender name:', 'themerex'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_sender_email">
								<label for="emailer_sender_email"><?php esc_html_e('Sender email:', 'themerex'); ?></label>
				...
				<div class="trx_emailer_buttons">
					<a href="#" id="trx_emailer_send"><?php echo esc_html_e('Send', 'themerex'); ?></a>
				</div>
	...
	function themerex_callback_emailer_submit() {
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email]))
				$response['error'] = esc_html__('E-mail address already in the subscribers list!', 'themerex');
			else {
				$subscribers[$group][$email] = md5(mt_rand());
				update_option('themerex_emailer_subscribers', $subscribers);
				$subj = sprintf(esc_html__('Site %s - Subscribe confirmation', 'themerex'), get_bloginfo('site_name'));
				...
				if (!@$mail($email, $subj, nl2br($msg), $headers)) {
					$response['error'] = esc_html__('Error send message!', 'themerex');
				}
				remove_filter( 'wp_mail_content_type', 'themerex_set_html_content_type' );
			}
		} else
			$response['error'] = esc_html__('E-mail address is not valid!', 'themerex');
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('themerex_emailer_subscribers', $subscribers);
					themerex_set_system_message(esc_html__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'themerex'), 'success');
		
					wp_safe_redirect( home_url('/') );
				} else if ($subscribers[$group][$email] != '') {
					themerex_set_system_message(esc_html__('Bad confirmation code!', 'themerex'), 'error');
				
					wp_safe_redirect( home_url('/') );
				} else {
					themerex_set_system_message(esc_html__('E-mail address already exists in the subscribers list!', 'themerex'), 'error');
