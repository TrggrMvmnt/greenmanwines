...
if ( !function_exists( 'themerex_template_services_1_theme_setup' ) ) {
	...
	function themerex_template_services_1_theme_setup() {
		themerex_add_template(array(
			...
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'themerex'),
		...
		?>
			<div<?php echo balanceTags($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				<div class="sc_services_item_content">
					...
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h4 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php echo balanceTags($post_data['post_title']); ?></a></h4><?php
						} else {
							?><h4 class="sc_services_item_title"><?php echo balanceTags($post_data['post_title']); ?></h4><?php
						}
					}
					?>
