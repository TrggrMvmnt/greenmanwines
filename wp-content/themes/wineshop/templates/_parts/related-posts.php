...
if (themerex_get_custom_option("show_post_related") == 'yes') {
	...
	if ($number > 0) {
		...
		$need_dummy = false;
		?>
		<section class="related_wrap<?php echo balanceTags($columns>1 ? '' : ' scroll_wrap') . esc_attr(themerex_get_template_property('related', 'container_classes')); ?>">

			<?php if ($need_wrap) themerex_open_wrapper('<div class="content_wrap">'); ?>
			
			<h2 class="section_title"><?php echo apply_filters('themerex_filter_related_posts_title', esc_html__('Related Posts', 'themerex'), $post_data['post_type']); ?></h2>
