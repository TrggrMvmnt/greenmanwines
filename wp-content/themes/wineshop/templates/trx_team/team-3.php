...
if ( !function_exists( 'themerex_template_team_3_theme_setup' ) ) {
	...
	function themerex_template_team_3_theme_setup() {
		themerex_add_template(array(
			...
			'title'  => esc_html__('Team /Style 3/', 'themerex'),
		...
		?>
			<div<?php echo balanceTags($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				<div class="sc_team_item_info">
					...
					<div class="sc_team_item_description"><?php echo trim(themerex_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? 100 : themerex_get_custom_option('post_excerpt_maxlength_masonry'))); ?></div>
				</div>
			</div>
		<?php
