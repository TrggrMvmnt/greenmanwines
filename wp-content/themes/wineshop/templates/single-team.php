...
if ( !function_exists( 'themerex_template_single_team_theme_setup' ) ) {
	...
	function themerex_template_single_team_theme_setup() {
		themerex_add_template(array(
			...
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'themerex'),
...
if ( !function_exists( 'themerex_template_single_team_output' ) ) {
	function themerex_template_single_team_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && themerex_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="headline" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php echo balanceTags($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
			<?php 
		}
		...
		if (!$post_data['post_protected'] && (
			...
			if (!empty($post_options['dedicated'])) {
				echo balanceTags($post_options['dedicated']);
			} else {
				themerex_enqueue_popup();
				?>
				<div class="post_thumb" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					<a class="hover_icon hover_icon_view" href="<?php echo esc_url($post_data['post_attachment']); ?>" title="<?php echo esc_attr($post_data['post_title']); ?>"><?php echo balanceTags($post_data['post_thumb']); ?></a>
				</div>
				<?php 
			}
			?>
		...
		if ($show_title && $post_options['location'] != 'center' && themerex_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="name" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php echo balanceTags($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
			<?php 
		}
		...
		if ($post_data['post_protected']) { 
			...
			echo trim(themerex_gap_wrapper(themerex_reviews_wrapper($post_data['post_content'])));
			...
			if ( themerex_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'themerex'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
