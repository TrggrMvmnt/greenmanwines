...
if ( !function_exists( 'themerex_template_single_standard_theme_setup' ) ) {
	...
	function themerex_template_single_standard_theme_setup() {
		themerex_add_template(array(
			...
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'themerex'),
			'thumb_title'  => esc_html__('Fullwidth image', 'themerex'),
			'w'		 => 1170,
			'h'		 => 660
		));
	}
...
if ( !function_exists( 'themerex_template_single_standard_output' ) ) {
	function themerex_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && themerex_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="<?php echo balanceTags($avg_author > 0 || $avg_users > 0 ? 'itemReviewed' : 'headline'); ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php echo balanceTags($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
		<?php 
		}
