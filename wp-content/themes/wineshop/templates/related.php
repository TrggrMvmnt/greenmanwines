...
if ( !function_exists( 'themerex_template_related_theme_setup' ) ) {
	...
	function themerex_template_related_theme_setup() {
		themerex_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /no columns/', 'themerex'),
		...
		themerex_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /2 columns/', 'themerex'),
		...
		themerex_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /3 columns/', 'themerex'),
			'thumb_title'  => esc_html__('Medium square image (crop)', 'themerex'),
			'w'		 => 390,
			'h'		 => 390
		));
		themerex_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /4 columns/', 'themerex'),
...
if ( !function_exists( 'themerex_template_related_output' ) ) {
	function themerex_template_related_output($post_options, $post_data) {
		...
		?>
		<<?php echo balanceTags($tag); ?> class="post_item post_item_<?php echo esc_attr($style); ?> post_item_<?php echo esc_attr($post_options['number']); ?>">
				...
				<?php if ($show_title) { ?>
					<div class="post_content_wrap">
						...
						if (!isset($post_options['links']) || $post_options['links']) { 
							?><h4 class="post_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php echo balanceTags($post_data['post_title']); ?></a></h4><?php
						} else {
							?><h4 class="post_title"><?php echo balanceTags($post_data['post_title']); ?></h4><?php
						}
