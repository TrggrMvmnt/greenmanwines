...
if ( !function_exists( 'themerex_template_clients_2_theme_setup' ) ) {
	...
	function themerex_template_clients_2_theme_setup() {
		themerex_add_template(array(
			...
			'title'  => esc_html__('Clients /Style 2/', 'themerex'),
	...
	function themerex_template_clients_2_output($post_options, $post_data) {
		...
		?>
			<div<?php echo balanceTags($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				class="sc_clients_item sc_clients_item_<?php echo esc_attr($post_options['number']) . ($post_options['number'] % 2 == 1 ? ' odd' : ' even') . ($post_options['number'] == 1 ? ' first' : '') . (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo balanceTags($post_options['tag_css']!='' ? ' style="'.esc_attr($post_options['tag_css']).'"' : '') . (!themerex_param_is_off($post_options['tag_animation']) ? ' data-animation="'.esc_attr(themerex_get_animation_classes($post_options['tag_animation'])).'"' : ''); ?>>
				<div class="sc_client_image"><?php echo trim($post_options['client_image']); ?>
					<div class="sc_client_hover">
						<div class="sc_client_info">
							...
							<h5 class="sc_client_title"><?php echo balanceTags($post_options['client_link'] ? '<a href="'.esc_url($post_options['client_link']).'">' : '') . ($post_options['client_name'] ? $post_options['client_name'] : $post_data['post_title']) . ($post_options['client_link'] ? '</a>' : ''); ?></h5>
