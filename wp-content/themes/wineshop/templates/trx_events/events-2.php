...
if ( !function_exists( 'themerex_template_events_2_theme_setup' ) ) {
	...
	function themerex_template_events_2_theme_setup() {
		themerex_add_template(array(
			...
			'template' => 'events-2',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 2/', 'themerex')
		));
	}
}
		...
		<div<?php echo balanceTags($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
			...
			if ($show_title) {
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?><h6 class="sc_events_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php echo balanceTags($post_data['post_title']); ?></a></h6><?php
				} else {
					?><h6 class="sc_events_item_title"><?php echo balanceTags($post_data['post_title']); ?></h6><?php
				}
			}
			?><span class="sc_events_item_time"><?php
				echo (trim($start_date[1]) ? $start_date[1] : esc_html__('Whole day', 'themerex'))
