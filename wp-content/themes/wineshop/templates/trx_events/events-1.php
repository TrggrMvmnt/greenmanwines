...
if ( !function_exists( 'themerex_template_events_1_theme_setup' ) ) {
	...
	function themerex_template_events_1_theme_setup() {
		themerex_add_template(array(
			...
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'themerex'),
				...
				<div class="sc_events_item_content">
					...
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h5 class="sc_events_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php echo balanceTags($post_data['post_title']); ?></a></h5><?php
						} else {
							?><h5 class="sc_events_item_title"><?php echo balanceTags($post_data['post_title']); ?></h5><?php
						}
					}
					?>
