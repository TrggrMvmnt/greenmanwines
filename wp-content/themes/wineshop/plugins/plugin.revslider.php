...
if (!function_exists('themerex_revslider_theme_setup')) {
	...
	function themerex_revslider_theme_setup() {
		if (themerex_exists_revslider()) {
			...
			if (is_admin()) {
				add_filter( 'themerex_filter_importer_options',			'themerex_revslider_importer_set_options' );
				add_action( 'themerex_action_importer_params',			'themerex_revslider_importer_show_params', 10, 1 );
				add_action( 'themerex_action_importer_clear_tables',	'themerex_revslider_importer_clear_tables', 10, 2 );
				add_action( 'themerex_action_importer_import',			'themerex_revslider_importer_import', 10, 2 );
				add_action( 'themerex_action_importer_import_fields',	'themerex_revslider_importer_import_fields', 10, 1 );
			}
		}
		if (is_admin()) {
			add_filter( 'themerex_filter_importer_required_plugins',	'themerex_revslider_importer_required_plugins', 10, 2 );
	...
	function themerex_revslider_settings_theme_setup2() {
		if (themerex_exists_revslider()) {
			...
			themerex_array_insert_after($THEMEREX_GLOBALS['options'], 'slider_engine', array(
				...
				"slider_alias" => array(
					"title" => esc_html__('Revolution Slider: Select slider',  'themerex'),
					"desc" => wp_kses( __("Select slider to show (if engine=revo in the field above)", 'themerex'), $THEMEREX_GLOBALS['allowed_tags'] ),
...
if ( !function_exists( 'themerex_revslider_required_plugins' ) ) {
	...
	function themerex_revslider_required_plugins($list=array()) {
		$list[] = array(
					...
					'slug' 		=> 'revslider',
					'source'	=> themerex_get_file_dir('plugins/install/revslider.zip'),
					'required' 	=> false
				);
...
if ( !function_exists( 'themerex_revslider_importer_required_plugins' ) ) {
	//add_filter( 'themerex_filter_importer_required_plugins',	'themerex_revslider_importer_required_plugins', 10, 2 );
	function themerex_revslider_importer_required_plugins($not_installed='', $importer=null) {
		if ($importer && in_array('revslider', $importer->options['required_plugins']) && !themerex_exists_revslider() )
...
if ( !function_exists( 'themerex_revslider_importer_set_options' ) ) {
	//add_filter( 'themerex_filter_importer_options',	'themerex_revslider_importer_set_options', 10, 1 );
	function themerex_revslider_importer_set_options($options=array()) {
		if (is_array($options)) {
...
if ( !function_exists( 'themerex_revslider_importer_show_params' ) ) {
	//add_action( 'themerex_action_importer_params',	'themerex_revslider_importer_show_params', 10, 1 );
	function themerex_revslider_importer_show_params($importer) {
		?>
		<input type="checkbox" <?php echo in_array('revslider', $importer->options['required_plugins']) ? 'checked="checked"' : ''; ?> value="1" name="import_revslider" id="import_revslider" /> <label for="import_revslider"><?php esc_html_e('Import Revolution Sliders', 'themerex'); ?></label><br>
		<?php
	}
}

// Clear tables
if ( !function_exists( 'themerex_revslider_importer_clear_tables' ) ) {
	//add_action( 'themerex_action_importer_clear_tables',	'themerex_revslider_importer_clear_tables', 10, 2 );
	function themerex_revslider_importer_clear_tables($importer, $clear_tables) {
		if (themerex_strpos($clear_tables, 'revslider')!==false && $importer->last_slider==0) {
			if ($importer->options['debug']) dfl(esc_html__('Clear Revolution Slider tables', 'themerex'));
			global $wpdb;
			$res = $wpdb->query("TRUNCATE TABLE " . esc_sql($wpdb->prefix) . "revslider_sliders");
			if ( is_wp_error( $res ) ) dfl( esc_html__( 'Failed truncate table "revslider_sliders".', 'themerex' ) . ' ' . ($res->get_error_message()) );
			$res = $wpdb->query("TRUNCATE TABLE " . esc_sql($wpdb->prefix) . "revslider_slides");
			if ( is_wp_error( $res ) ) dfl( esc_html__( 'Failed truncate table "revslider_slides".', 'themerex' ) . ' ' . ($res->get_error_message()) );
			$res = $wpdb->query("TRUNCATE TABLE " . esc_sql($wpdb->prefix) . "revslider_static_slides");
			if ( is_wp_error( $res ) ) dfl( esc_html__( 'Failed truncate table "revslider_static_slides".', 'themerex' ) . ' ' . ($res->get_error_message()) );
		}
	}
}

// Import posts
if ( !function_exists( 'themerex_revslider_importer_import' ) ) {
	//add_action( 'themerex_action_importer_import',	'themerex_revslider_importer_import', 10, 2 );
	function themerex_revslider_importer_import($importer, $action) {
		if ( $action == 'import_revslider' ) {
			if (file_exists(WP_PLUGIN_DIR.'/revslider/revslider.php')) {
				...
				$dir = themerex_get_folder_dir($importer->options['folder_with_revsliders']);
				if ( is_dir($dir) ) {
					$hdir = @opendir( $dir );
					if ( $hdir ) {
						if ($importer->options['debug']) dfl( esc_html__('Import Revolution sliders', 'themerex') );
						...
						for ($i=0; $i<count($sliders); $i++) {
							...
							if ($importer->options['debug']) dfl( sprintf(esc_html__('Process slider "%s"', 'themerex'), $sliders[$i]['name']) );
							if (!is_array($_FILES)) $_FILES = array();
							$_FILES["import_file"] = array("tmp_name" => $sliders[$i]['path']);
							$response = $slider->importSliderFromPost();
							if ($response["success"] == false) {
								$msg = sprintf(esc_html__('Revolution Slider "%s" import error', 'themerex'), $sliders[$i]['name']);
								$importer->response['error'] = $msg;
								dfl( $msg );
								dfo( $response );
							} else {
								if ($importer->options['debug']) dfl( sprintf(esc_html__('Slider "%s" imported', 'themerex'), $sliders[$i]['name']) );
							}
							break;
						}
						// Write last slider into log
						themerex_fpc($importer->import_log, $i+1 < count($sliders) ? '0|100|'.($i+1) : '');
						$importer->response['result'] = min(100, round(($i+1) / count($sliders) * 100));
					}
				}
			} else {
				dfl( sprintf(esc_html__('Can not locate plugin Revolution Slider: %s', 'themerex'), WP_PLUGIN_DIR.'/revslider/revslider.php') );
			}
		}
	}
}
...
if ( !function_exists( 'themerex_revslider_importer_import_fields' ) ) {
	//add_action( 'themerex_action_importer_import_fields',	'themerex_revslider_importer_import_fields', 10, 1 );
	function themerex_revslider_importer_import_fields($importer) {
		?>
		<tr class="import_revslider">
			<td class="import_progress_item"><?php esc_html_e('Revolution Slider', 'themerex'); ?></td>
...
if ( !function_exists( 'themerex_revslider_list_sliders' ) ) {
	...
	function themerex_revslider_list_sliders($list=array()) {
		$list["revo"] = esc_html__("Layer slider (Revolution)", 'themerex');
		return $list;
	}
}
