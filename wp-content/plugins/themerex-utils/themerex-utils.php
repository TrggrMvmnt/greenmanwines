...
if (!function_exists('themerex_utils_require_data')) {	
	function themerex_utils_require_data($type, $name, $args) {
		if ($type == 'taxonomy')
...
if (!function_exists('themerex_utils_require_shortcode')) {	
	function themerex_utils_require_shortcode($name, $callback) {
...
/* Twitter API
------------------------------------------------------ */
if (!function_exists('themerex_utils_twitter_acquire_data')) {
	function themerex_utils_twitter_acquire_data($cfg) {
		...
		if (!$data) {
			...
			$code = $tmhOAuth->user_request(array(
				'url' => $tmhOAuth->url(themerex_utils_twitter_mode_url($cfg['mode']))
			));
			if ($code == 200) {
				$data = json_decode($tmhOAuth->response['response'], true);
				if (isset($data['status'])) {
					$code = $tmhOAuth->user_request(array(
						'url' => $tmhOAuth->url(themerex_utils_twitter_mode_url($cfg['oembed'])),
						'params' => array(
...
if (!function_exists('themerex_utils_twitter_mode_url')) {
	function themerex_utils_twitter_mode_url($mode) {
		$url = '/1.1/statuses/';
...
// Compile less-files
if (!function_exists('themerex_utils_less_compiler')) {	
	function themerex_utils_less_compiler($list, $opt) {
		...
		foreach($list as $file) {
			...
			try {
				...
				if ($opt['compiler'] == 'lessc') {
					$parser = new lessc;
					//$parser->registerFunction("replace", "themerex_utils_less_func_replace");
				...
				if ($css) {
					if ($opt['map']=='no') {
						...
						if ($file == get_template_directory(). '/style.less') {
							...
							$css =	"/*"
									. "\n"
									. __('Attention! Do not modify this .css-file!', 'themerex_utils') 
									. "\n"
									. __('Please, make all necessary changes in the corresponding .less-file!', 'themerex_utils')
									. "\n"
									. "*/"
									. "\n"
									. '@charset "utf-8";'
...
// LESS function
/*
if (!function_exists('themerex_utils_less_func_replace')) {	
	function themerex_utils_less_func_replace($arg) {
    	return $arg;
	}
}
*/
